...
def setup_world_panel(
    ...
    patches = [
        ...
        mpatches.Patch(color=c.dead, label="Muerto")
    ]
...
def setup_sir_panel(
    ...
    p: SimulationParams,
    c: Colors,
    sirvd_vals: List[Tuple[int, int, int, int, int]]
) -> dict:
    ...
    ax.set_xlim(0, len(sirvd_vals) - 1)
    ...
    # Extraer valores
    S_vals = [x[0] for x in sirvd_vals]
    I_vals = [x[1] for x in sirvd_vals]
    R_vals = [x[2] for x in sirvd_vals]
    V_vals = [x[3] for x in sirvd_vals]
    D_vals = [x[4] for x in sirvd_vals]
    taxis = list(range(len(sirvd_vals)))
    ...
    line_d, = ax.plot([], [], color=c.dead, lw=2.5, label="Muertos", zorder=5)
...
def add_title(fig: plt.Figure, p: SimulationParams) -> None:
    ...
    fig.suptitle(
        ...
        f"Dur.inf={p.infection_duration}  Vacc.={p.vaccination_rate}  Mort.={p.mortality_rate}",
